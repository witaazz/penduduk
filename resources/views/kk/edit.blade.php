...
<div class="bg-white-100 dark:bg-white-100">
    <div class="w-full max-w-3xl mx-auto p-8">
        <div class="bg-white  p-8 rounded-lg shadow-md border ">
            ...
            <form method="POST" action="{{ route('updatekartukeluarga', $kk->id) }}" enctype="multipart/form-data">
                @csrf
                <div class="mb-5">
                    <label for="nokk" class="mb-3 block text-base font-medium text-[#07074D]">
                        No KK
                    </label>
                    <input type="text" name="nokk" id="nokk" value="{{ $kk->nokk }}" class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" />
                    <span>
